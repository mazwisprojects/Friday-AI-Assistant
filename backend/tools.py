...
build_custom_tool = {"name": "build_custom_tool", "description": "Builds and behavior-tests a governed custom Friday tool. New capabilities start pending review until approved. Valid operations: 'readonly_powershell' (run a read-only PowerShell command), 'http_json_get' (fetch JSON from a URL), 'python_module' (run a Python script). For system checks like Windows updates, use 'readonly_powershell'.", "parameters": {"type": "OBJECT", "properties": {"name": {"type": "STRING"}, "description": {"type": "STRING"}, "operation": {"type": "STRING", "description": "One of: readonly_powershell, http_json_get, python_module"}, "parameters": {"type": "OBJECT"}, "config": {"type": "OBJECT"}, "governance": {"type": "OBJECT", "description": "permissions, dependencies, resource_limits, test_fixtures, expiry metadata"}}, "required": ["name", "description", "operation"]}}
