...
import threading
import traceback
from actions.self_maintenance import record_tool_failure
...
class PluginManager:
    ...
    def register_startup_agents(self) -> dict:
        ...
        for path in sorted(self.backend_dir.glob("agents/*.py")):
            ...
            if module_name.startswith("__") or module_name in registered:
                continue
            known_manifest = self.agent_builder.agents.get(module_name)
            if known_manifest is not None:
                continue
            if path.read_text(encoding="utf-8").startswith("# Generated by Friday's verified agent builder."):
                continue
            try:
                ...
                skipped = [item for item in skipped if item != module_name]
            except Exception as error:
                skipped.append(f"{module_name}: {error}")
        return {"registered": registered, "skipped": skipped}

    def load_all_tools(self) -> dict:
        """Load all active tools into the live callable registry.

        Tools are loaded safely - a single broken tool is skipped with a warning
        and recorded in the failure log, rather than crashing the whole load.
        """
        loaded = []
        failed = []
        with self._lock:
            for name, manifest in list(self.tool_builder.tools.items()):
                if not is_active(manifest):
                    continue
                try:
                    module_path = self.backend_dir / manifest["module_path"]
                    if not module_path.exists():
                        raise FileNotFoundError(f"Module file missing: {module_path}")
                    self.tool_builder._load_and_instantiate(name, module_path)
                    loaded.append(name)
                except Exception as exc:
                    failed.append(name)
                    logger.warning("Failed to load tool %s: %s", name, exc)
                    record_tool_failure(name, traceback.format_exc())
